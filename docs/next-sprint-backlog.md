...
### FEAT-10.5: 사용자 역할 세그먼트 (학생/학부모/선생님)
| 항목 | 내용 |
|------|------|
| 가치 | **중간** — 역할별 UX 최적화, 선생님 바이럴 효과 기대 |
| 난이도 | **낮음~중간** |
| 우선순위 | **P2** |
| 전제조건 | Sprint 1 DB 인프라 + 실제 사용자 데이터(GA) 분석 후 결정 |
| DB 테이블 | `user_settings.role` 컬럼 추가 ('student' / 'parent' / 'teacher') |
| 설명 | 회원가입 시 역할 선택 → 마이페이지 진입점/대시보드 차별화. **전용 페이지 분리는 하지 않음** — 같은 기능을 역할별 톤/우선순위만 다르게 표시 |
| 역할별 차이점 | 학생: 역방향 검색·시너지 맵 우선 노출 / 학부모: 적합도 점수·갭 분석 강조 / 선생님: 일괄 비교·데이터 내보내기(향후) |
| 주의사항 | ⚠️ 배포 후 GA 데이터로 실제 세그먼트 비율 확인 후 투자 범위 결정. 사용자 검증 전 전용 페이지 개발 금지 |
| 영향 페이지 | 마이페이지, 회원가입 플로우 |
...
| **Sprint 3** | FEAT-08~10.5 | 개인화 심화 + 대시보드 + 역할 세그먼트 |
